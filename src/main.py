...
# Define the header
header = ['donor_id', 'name', 'birthdate', 'age_at_donation', 'sex', 'ethnicity', 'blood_type', 'first_time_donor', 'donation_datetime']
...
for week, count in adjusted_counts.items():
    ...
    if len(week_donations) > count:
        ...
        additional_donors = week_donations.sample(count - len(week_donations), replace=True)
...
def generate_donation_datetime_for_week(week_start, count):
    ...
    random_times = [timedelta(hours=np.random.randint(0, 24), minutes=np.random.randint(0, 60)) for _ in range(count)]
    return [week_start + timedelta(days=int(day)) + time for day, time in zip(random_days, random_times)]

new_donation_datetimes = []
for week in adjusted_counts.index:
    week_start = week.start_time
    donation_datetimes = generate_donation_datetime_for_week(week_start, adjusted_counts[week])
    new_donation_datetimes.extend(donation_datetimes)

# Ensure the length matches
if len(new_donation_datetimes) != len(adjusted_donors_df):
    raise ValueError("Length of generated donation datetimes does not match the adjusted donors DataFrame")
